...
def minimum_filter1d(red, green, blue, size):
    redGaussian =  ndimage.minimum_filter1d(red, size=size)
    greenGaussian = ndimage.minimum_filter1d(green, size=size)
    blueGaussian =  ndimage.minimum_filter1d(blue, size=size)
    return np.dstack((redGaussian, greenGaussian, blueGaussian))

varrays = []
harrays = []
for i in range(1, 10):
    gaussian3D = minimum_filter1d(red, green, blue, i)
...
Image.fromarray(full3D).save('minimum_filter1d.png')