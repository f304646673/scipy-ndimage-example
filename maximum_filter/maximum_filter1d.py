...
def maximum_filter1d(red, green, blue, size):
    redGaussian =  ndimage.maximum_filter1d(red, size=size)
    greenGaussian = ndimage.maximum_filter1d(green, size=size)
    blueGaussian =  ndimage.maximum_filter1d(blue, size=size)
    return np.dstack((redGaussian, greenGaussian, blueGaussian))

varrays = []
harrays = []
for i in range(1, 10):
    gaussian3D = maximum_filter1d(red, green, blue, i)
...
Image.fromarray(full3D).save('maximum_filter1d.png')